        ...
        <div class="flex items-start justify-between gap-4 flex-wrap">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-slate-600 mt-0.5">
              ...
              ${job.salary ? ` · <span class="text-emerald-600">${escapeHtml(job.salary)}</span>` : ""}
            </p>
            <p class="text-xs text-slate-400 mt-0.5">
              ${escapeHtml(job.source)} · Posted ${fmtDate(job.posted_at) || fmtDate(job.scraped_at)}
            </p>
            ${job.description ? `<p class="text-sm text-slate-700 mt-2 line-clamp-3">${escapeHtml(job.description)}</p>` : ""}
            ${tags ? `<div class="mt-2 flex gap-1 flex-wrap">${tags}</div>` : ""}
          </div>
          <div class="flex flex-col gap-2 shrink-0">
            <a href="${escapeHtml(job.url)}" target="_blank" rel="noopener noreferrer"
               class="text-xs text-center px-3 py-1.5 rounded-md border border-slate-300 hover:bg-slate-100">
              View →
            </a>
